...
/** 비서실장 이름 — company.config.ts 를 고치면 대사도 같이 바뀐다.
 *  예전엔 템플릿 인물 이름이 코드에 박혀 있어서, 설정에 없는 사람이 채팅에서 말했다. */
const SECRETARY = DEPT_LEAD.secretary?.name ?? "비서실장";

/** 승인 대기 중 회의실에 앉아 있는 세 명 — 제안전략·교안설계·비서실 팀장 */
 ...
 *  팀장 이름은 설정에서 자동으로 붙는다(아래 buildDeptKeywords). */
const DEPT_KEYWORD_SEED: [string, string[]][] = [
  ["qa", ["qa", "큐아", "검수", "품질", "톤"]],
  ["brand", ["블로그", "채널", "네이버", "포스팅", "seo"]],
  ["strategy1", ["제안", "제안서", "전략", "기획", "커리큘럼", "견적", "톱3", "top 3"]],
  ["strategy2", ["교안", "강의 설계", "모듈", "학습 활동", "스크립트"]],
  ["research", ["리서치", "조사", "뉴스", "공고", "동향", "시장조사"]],
  ["reels", ["교재", "워크북", "핸드아웃", "평가지", "실습지"]],
  ["carousel", ["카드뉴스", "인스타", "캐러셀", "캡션", "canva", "칸바"]],
  ["partner", ["프로필", "브랜딩", "이력", "소개서", "강사 소개"]],
  ["finance", ["재무", "세무", "정산", "입금", "강사료", "세금", "돈"]],
  ["review", ["성과", "리뷰", "지표", "후기", "만족도"]],
  ["ops", ["자동화", "운영팀", "스케줄", "연동", "서버", "볼트", "옵시디언"]],
  ["secretary", ["비서", "비서실", "브리핑"]],
];

/** 설정에 적힌 팀장 이름·콜사인으로 검색어를 채운다 */
const DEPT_KEYWORDS: [string, string[]][] = DEPT_KEYWORD_SEED.map(([id, words]) => {
  const lead = DEPT_LEAD[id];
  const extra = [lead?.name, lead?.callsign].filter(Boolean) as string[];
  return [id, [...words, ...extra.map((w) => w.toLowerCase())]];
});
...
export class Company {
  ...
  private *dayScript(): Generator<number | (() => boolean), void, void> {
    ...
    this.pushLog("📋", `비서실 최종 브리핑 완료 — 완료 ${stats.done}팀 · 연동 대기 ${stats.blocked}팀`, "pink");
    yield 3;
    this.stand(seri);
    this.sitAtDesk(seri);
    yield this.allFree([seri]);
  ...
  private reportDelay() {
    ...
    if (this.approvalPending) {
      ...
      lines.push(
        `원인은 하나예요 — 대표님 결재 대기입니다. 회의실에서 ${MEETING_TRIO}가 ${waited}초째 기다리고 있어요.`,
      );
      lines.push("승인만 눌러주시면 바로 교안 설계로 넘어갑니다.");
    }
