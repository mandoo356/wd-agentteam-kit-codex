/**
 * 완료 보고 발행 — 같은 내용을 Notion(비서실 일일 브리핑)과 Discord로 동시에 보낸다.
 *
 * 비밀값은 코드에 두지 않는다. 로컬은 `.dev.vars`, 배포는 `wrangler secret put`.
 *   NOTION_TOKEN          Notion 내부 통합 토큰 (ntn_…)
 *   NOTION_BRIEFING_DB    비서실 일일 브리핑 데이터베이스 ID
...
async function sendDiscord(report: DayReport, env: PublishEnv, notionUrl?: string): Promise<TargetResult> {
  ...
  const embed = {
    ...
    footer: { text: notionUrl ? "Notion에도 저장됨 · My AI Office" : "My AI Office" },
  ...
  const response = await fetch(env.DISCORD_WEBHOOK_URL, {
    ...
    body: JSON.stringify({ username: "귀요미매니저", content: "📋 오늘 전사 브리핑입니다.", embeds: [embed] }),
  });
